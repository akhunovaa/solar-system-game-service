package com.botmasterzzz.solar.web.rest;
...
import com.botmasterzzz.solar.config.KafkaProperties;
...
class NnSolarSystemKafkaResourceIT {
    ...
    void setup() {
        ...
        NnSolarSystemKafkaResource kafkaResource = new NnSolarSystemKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/nn-solar-system-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/nn-solar-system-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
